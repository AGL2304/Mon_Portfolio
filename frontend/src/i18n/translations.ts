...
const FR_DICT = {
  ...
  spot: {
    ...
    title: "PURPLE TEAM",
    pitch:
      "Attaquer pour mieux défendre. Je passe de l'<em>exploitation</em> à la <em>détection</em>, puis à la <em>remédiation</em>.",
    termTitle: "agl23@purpleteam: ~/engagement",
    script: [
      { k: "prompt", c: "nmap -sV 10.10.14.7" },
      { k: "out", c: "PORT     STATE  SERVICE   VERSION" },
      { k: "out", c: "22/tcp   open   ssh       OpenSSH 8.2" },
      { k: "crit", c: "80/tcp   open   http      vuln: SQLi détectée" },
      { k: "prompt", c: "sqlmap -u http://target/login --batch" },
      { k: "warn", c: "[*] payload injecté : ' OR 1=1 --" },
      { k: "ok", c: "[+] accès obtenu // dump utilisateurs" },
      { k: "prompt", c: "# bascule BLUE TEAM" },
      { k: "out", c: "sigma > règle de détection générée" },
      { k: "ok", c: "[+] alerte SIEM | IP blocklistée | CVSS 9.8" },
      { k: "prompt", c: "rapport --export pdf --remediation" },
      { k: "ok", c: "[+] plan de remédiation priorisé : OK" },
    ],
    termTitle2: "agl23@purpleteam: ~/threat-hunting",
    script2: [
      { k: "prompt", c: "tail -f /var/log/auth.log | grep -i fail" },
      { k: "out", c: "Failed password for root from 45.13.x.x" },
      { k: "warn", c: "[!] 240 échecs SSH en 60s : brute force" },
      { k: "prompt", c: "# corrélation CTI" },
      { k: "out", c: "ioc > 45.13.x.x : AbuseIPDB 100/100" },
      { k: "crit", c: "[X] IP liée au botnet Mirai (threat feed)" },
      { k: "prompt", c: "fail2ban-client set sshd banip 45.13.x.x" },
      { k: "ok", c: "[+] IP bannie | règle iptables ajoutée" },
      { k: "prompt", c: "# durcissement" },
      { k: "ok", c: "[+] MFA imposé | SSH par clé | port modifié" },
      { k: "ok", c: "[+] rapport CTI exporté : OK" },
    ],
  },
  grc: {
...
const EN_DICT: DictShape = {
  ...
  spot: {
    ...
    title: "PURPLE TEAM",
    pitch:
      "Attack to defend better. I move from <em>exploitation</em> to <em>detection</em>, then <em>remediation</em>.",
    termTitle: "agl23@purpleteam: ~/engagement",
    script: [
      { k: "prompt", c: "nmap -sV 10.10.14.7" },
      { k: "out", c: "PORT     STATE  SERVICE   VERSION" },
      { k: "out", c: "22/tcp   open   ssh       OpenSSH 8.2" },
      { k: "crit", c: "80/tcp   open   http      vuln: SQLi found" },
      { k: "prompt", c: "sqlmap -u http://target/login --batch" },
      { k: "warn", c: "[*] payload injected: ' OR 1=1 --" },
      { k: "ok", c: "[+] access gained // user dump" },
      { k: "prompt", c: "# switch to BLUE TEAM" },
      { k: "out", c: "sigma > detection rule generated" },
      { k: "ok", c: "[+] SIEM alert | IP blocklisted | CVSS 9.8" },
      { k: "prompt", c: "report --export pdf --remediation" },
      { k: "ok", c: "[+] prioritized remediation plan: OK" },
    ],
    termTitle2: "agl23@purpleteam: ~/threat-hunting",
    script2: [
      { k: "prompt", c: "tail -f /var/log/auth.log | grep -i fail" },
      { k: "out", c: "Failed password for root from 45.13.x.x" },
      { k: "warn", c: "[!] 240 SSH failures in 60s: brute force" },
      { k: "prompt", c: "# CTI correlation" },
      { k: "out", c: "ioc > 45.13.x.x : AbuseIPDB 100/100" },
      { k: "crit", c: "[X] IP tied to Mirai botnet (threat feed)" },
      { k: "prompt", c: "fail2ban-client set sshd banip 45.13.x.x" },
      { k: "ok", c: "[+] IP banned | iptables rule added" },
      { k: "prompt", c: "# hardening" },
      { k: "ok", c: "[+] MFA enforced | SSH key-only | port changed" },
      { k: "ok", c: "[+] CTI report exported: OK" },
    ],
  },
  grc: {
