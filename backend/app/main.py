...
if not settings.cors_origins:
    raise RuntimeError("CORS_ORIGINS doit etre defini explicitement (liste d'origines autorisees).")
...
async def upload_cv(file: UploadFile = File(...), db: Session = Depends(get_db)) -> UploadResponse:
    ...
    return UploadResponse(url=cv_url, filename=file.filename, content_type="application/pdf")
...
def download_cv(db: Session = Depends(get_db)) -> Response:
    cv = get_cv(db)
    if cv is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="CV non disponible.")
    return Response(
        content=cv.data,
...
async def upload_photo(
    ...
    base_url = settings.app_base_url.rstrip("/")
    photo_url = f"{base_url}/api/public/photo" if base_url else "/api/public/photo"
...
def get_photo(db: Session = Depends(get_db)) -> Response:
    img = get_profile_image(db)
    if img is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Photo non disponible.")
    return Response(
        content=img.data,
